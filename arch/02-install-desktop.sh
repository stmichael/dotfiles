...
# desktop environment
sudo pacman -S hyprland hyprlock waybar socat hyprpolkitagent
...
# yubikey
sudo pacman -S libfido2 yubikey-manager pam-u2f
mkdir ~/.config/Yubico
...
sudo sed -i '2a\auth       sufficient   pam_u2f.so cue origin=pam://stmichaels-host appid=pam://stmichaels-host' /etc/pam.d/ly
sudo sed -i '3a\auth        sufficient  pam_u2f.so cue origin=pam://stmichaels-host appid=pam://stmichaels-host' /etc/pam.d/hyprlock
